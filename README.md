...
* **Training:** Trained on an RTX 3080 (24GB RAM) with a Ryzen 5 3600. This setup allowed a YOLO Nano model to converge in ~20 minutes, and a YOLO Small model in under an hour with robust spatial accuracy.
...
* **State Detection:** Using the Homography matrix, I projected the YOLO-detected coordinates onto the chronologic stock images. I then extracted a small patch under that coordinate and evaluated the HSV color space to detect when the yellow pallet adhesive was physically covered by a component.
...
Ultimately, projecting perfect high-resolution coordinates down to the stock images (homography) and utilising a targeted color-space probe yielded the most stable PoC.
